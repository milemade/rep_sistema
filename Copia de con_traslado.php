...
if($eliminacion==1) {//confirmacion de insercion  
	...
	$sql="SELECT * ,(SELECT cod_bod_sal_tras FROM m_traslado WHERE cod_tras=cod_mtras_dtra) AS bodega_salida, (SELECT cod_bod_ent_tras FROM m_traslado WHERE cod_tras=cod_mtras_dtra) AS bodega_entrada  FROM  d_traslado  where cod_mtras_dtra=$eli_codigo ";
	$dbser= new  Database();	
	$dbser->query($sql);
	while($dbser->next_row())
	{
		kardex("resta",$dbser->cod_ref_dtra,$dbser->bodega_entrada,$dbser->cant_dtra,0,$dbser->cod_pes_dtra);
		kardex("suma",$dbser->cod_ref_dtra,$dbser->bodega_salida,$dbser->cant_dtra,0,$dbser->cod_pes_dtra);	
	}
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$sql="SELECT * , (SELECT nom_bod FROM bodega WHERE cod_bod=cod_bod_sal_tras) AS bodega_salida, (SELECT nom_bod FROM bodega WHERE cod_bod=cod_bod_ent_tras) AS bodega_entrada FROM m_traslado $where  ORDER BY fec_tras DESC ";
