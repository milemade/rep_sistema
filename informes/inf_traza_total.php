...
<TR>
  <TD colspan="8" class="titulogran"><div align="center">ROTACION GENERAL </div></TD>
  </TR>
<TR>
...
//	echo $fecha_fin."===";
	...
	$where=" where num_fac > 0 ";
	
	if($cod_categoria > 0){
		$where.=" and cod_mar= $cod_categoria ";
	}
	
	if($tipo_producto > 0){
		$where.=" and d_factura.cod_tpro= $tipo_producto ";
	}
	
	if($peso > 0){
		$where.=" and cod_peso= $peso ";
	}
	
	//--------------
	
	if($producto > 0){
		$where.=" and producto.cod_pro= $producto  ";
	}
...
group by  d_factura.cod_pro ORDER BY cant_pro DESC ";
