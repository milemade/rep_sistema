...
$db1 = new Database();
$db2 = new Database();
//Consulta para buscar las facturas de los proveedores
$sql="SELECT DISTINCT (
				a.cod_prove_ment
				), b.nom_pro, b.ident_pro
				FROM m_entrada a
				JOIN proveedor b ON a.`cod_prove_ment` = b.`cod_pro` 
				WHERE a.remision =0
				ORDER BY b.nom_pro ASC;";
$db->query($sql);	
...
?>
	<TR>
		...
		<TD height="16" colspan="2" class="subfongris">NOMBRE PROVEEDOR: <?=strtoupper($db->nom_pro)?></TD>
		<TD height="16" colspan="3" class="subfongris" >NIT: <?=$db->ident_pro?></TD>
	...
	<?
	  echo $sql_33=" SELECT MONTH(fec_ment) as mes,    
	                   sum(m_entrada.total_ment) as total,
					   (select nom_mes from meses WHERE cod_mes= MONTH(fec_ment) ) as nombre_mes 
				  FROM m_entrada 
				  where cod_prove_ment='$cod_proveedor' and fec_ment BETWEEN '$fec_ini' AND '$fec_fin' group by cod_prove_ment , mes ";
$db2->query($sql_33);	
while($db2->next_row())
{
...
DATE_ADD(m_entrada.`fec_ment` , INTERVAL proveedor.`credito_pro` DAY))) as dias_faltantes , DATEDIFF(now(),m_entrada.`fec_ment` ) as dias_cartera , MONTH(fec_ment) as mes  from m_entrada  inner join proveedor on m_entrada.`cod_prove_ment`=proveedor.`cod_pro`  where cod_pro=$cod_proveedor and m_entrada.`est_ment`<>'CANCELADA' and  MONTH(fec_ment)='$codigo_mes'and fec_ment>'$fec_ini' and  fec_ment<'$fec_fin'  order by proveedor.cod_pro , mes ";
	...
	<TR>
		<!--<TD width="10%" height="12" class="subfongris">TIPO </TD>-->
		<TD height="16" colspan="3" class="ctablasup"><div align="left">Subtotal
          <?=$nombre_mes?>
        : &nbsp;</div></TD>
		<TD height="16" class="ctablasup"><span class="ctablasup">
		  <?=number_format($subtotal_mes,0,".",".")?>
		</span></TD>
		<TD height="16" class="ctablasup">&nbsp;</TD>
		<TD height="16" class="ctablasup">&nbsp;</TD>
		<!--<TD width="11%" class="botones1" >OPCIONES</TD>-->
	</TR>
	<TR>
