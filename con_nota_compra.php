...
if($eliminacion==1) {//confirmacion de insercion  
	$error=eliminar("notas_compra",$eli_codigo,"cod_notc");
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
  ...
  $sql="SELECT a.cod_notc, 
			   a.fec_notc, 
			   a.val_notc, 
			   a.cod_ven_notc, 
			   b.fac_ment
		 FROM notas_compra a
		 JOIN m_entrada b ON a.cod_ven_notc = b.cod_ment
		 ORDER BY b.fac_ment,a.fec_notc DESC "; 
...
<script type="text/javascript" src="inf.js"></script>
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_nota_compra.php">
                    ...
                    <tr>
                      <td bgcolor="#E9E9E9"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/page.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_nota_compra.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          ...
                          <td width="160" valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="a.fec_notc">Fecha de la nota</option>
							<option value="a.cod_ven_notc">Factura</option>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
                        <tr>
						  ...
						  <td   class="ctablasup">NUMERO FACTURA</td>
                          <td   class="ctablasup"  >OPCIONES</td>
                        </tr>
						<? 
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_nota_compra.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						//echo $sql;
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_notc' action='man_nota_compra_det_edicion.php'>  ";
							...
							echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\" onClick=\"imprimir_inf('inf_nota_compra.php',$db->cod_notc,'mediano')\" /></td>";	
							if ($eliminar==10)
...
<form name="forma" method="post" action="con_nota_compra.php">
...
<?php if($cantidad_paginas>0)$db->close(); ?>
