...
if($eliminacion==1) {//confirmacion de insercion  
	$error=eliminar("producto_descuento",$eli_codigo,"cod_des");
	if ($error >=1)
...
$sql="SELECT cod_des, cod_pro_des,cod_emp_des,por_des,cod_fry_pro,nom_pro,nom_rso FROM producto_descuento
INNER JOIN producto ON producto_descuento.cod_pro_des=producto.cod_pro
INNER JOIN rsocial ON producto_descuento.cod_emp_des=rsocial.cod_rso  $where "; 
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_descuento.php">
                    ...
                    <tr>
                      <td bgcolor="#E9E9E9"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/page.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_descuento.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          ...
                          <td width="160" valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="nom_pro">Nombre Producto</option>
							<option value="cod_fry_pro">Codigo Producto</option>
							<option value="nom_rso">Razon Social</option>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
                        <tr>
						  ...
						  <td width="81"  class="ctablasup">CODIGO PRO </td>
						  <td width="185"  class="ctablasup">NOMBRE PRODUCTO</td>
						  <td width="192"  class="ctablasup">RAZON SOCIAL </td>
						  <td width="38"  class="ctablasup">DESC</td>
						  
                          <td  class="ctablasup"  width="116">OPCIONES</td>
                        </tr>
						<? 
						
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_descuento.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_des' action='man_descuento.php'>  ";
                          	echo "<td >$db->cod_fry_pro</td>";
							echo "<td >$db->nom_pro </td>";
							echo "<td >".substr($db->nom_rso,0,25)."... </td>";
							echo "<td >".number_format($db->por_des,1,".",".")."%</td>";
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_des'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.gif' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_des.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.gif' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.gif' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_des) /></td> ";
							else
...
<form name="forma" method="post" action="cod_descuento.php">
