...
{ #codigo para buscar 
  ...
  $sql="SELECT * ,((pre_ven_pro/ 100) *iva_pro )AS total_iva , pre_ven_pro AS venta 
    ...
    $pag = $_GET['filtrocantidad'];
	$cantidad_paginas=paginar($sql);
	if ($pag!='todos'and $pag!='')
	{
	$cant_pag=ceil($cantidad_paginas/$pag);
	}
	if(!empty($act_pag)) 
	$inicio=($act_pag -1)*$pag;
	...
	$options =" limit  $inicio, 25";
	if (isset($pag))
	{
		if($pag !="todos")
		{
			$options = " limit  $inicio, {$pag}";
		}
		  else 
			$options ="";
	 }
...
<input type="hidden" id="datos_a_enviar" name="datos_a_enviar" />
    ...
    <form action="con_referencia.php" method="get">
			    <select name="filtrocantidad"  id="filtrocantidad" >
                <option value="25" <?php echo ($pag === "25") ? "selected='selected'" : ''?>>25</option>
                <option value="50" <?php echo ($pag === "50") ? "selected='selected'" : ''?>>50</option>
                <option value="100" <?php echo ($pag === "100") ? "selected='selected'" : ''?>>100</option>
                <option value="todos"  <?php echo ($pag === "todos") ? "selected='selected'" : ''?>>Todos</option>
                </select>
