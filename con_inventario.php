...
if($eliminacion==1) {//confirmacion de insercion  
	...
	$sql="select * from  ajuste  where cod_aju=$eli_codigo ";
	$dbser= new  Database();	
	$dbser->query($sql);
	if($dbser->next_row())
	{
	...
	$error=eliminar("ajuste",$eli_codigo,"cod_aju");
...
$sql="select distinct punto_venta.cod_bod as valor , nom_bod as nombre from punto_venta  inner join  bodega  on punto_venta.cod_bod=bodega.cod_bod where cod_ven=$global[2]";
		...
		{
			$where_cli .= "cod_bod_aju= ".$dbdatos->valor  ;
			$where_cli .= " or ";
		}		
		$where_cli .= " cod_bod_aju < 0 "; 


//echo $det."=";

if($det==0)
	$where.=" where ajuste.cod_aju>0   and  ( $where_cli )  ";
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$sql=" SELECT * FROM ajuste left JOIN producto ON  ajuste.cod_ref_aju  =producto.cod_pro LEFT JOIN 	tipo_producto ON producto.cod_tpro_pro = tipo_producto.cod_tpro LEFT JOIN  marca ON producto.cod_mar_pro = marca.cod_mar LEFT  JOIN peso ON producto.cod_pes_pro = peso.cod_pes LEFT JOIN bodega ON bodega.cod_bod=ajuste.cod_bod_aju  LEFT JOIN bodega1 ON bodega1.cod_bod=ajuste.cod_bod_aju  $where ORDER BY fec_aju DESC ";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_inventario.php">
                    ...
                    <tr>
                      <td bgcolor="#E9E9E9"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/page.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_inventario.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          <td width="160">
                          ...
                          <td width="160" valign="middle"><select name="campos" class="textfieldlista" id="campos" >
							...
							<option value="nom_tpro">Producto</option>
							<option value="cod_fry_pro">Codigo Producto</option>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
                        <tr>
						  ...
						  <td  class="ctablasup" >TIPO DE PRODUCTO </td>
						  <td  class="ctablasup" >PRODUCTO </td>
						...
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_inventario.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_aju' action='man_inventario.php'>  ";
                          	echo "<td >$db->fec_aju</td>";
							echo "<td >$db->nom_tpro</td>";
							echo "<td >$db->nom_pro </td>";
							echo "<td >$db->cod_fry_pro </td>";
							echo "<td >$db->nom_bod </td>";
							echo "<td align='right'>".number_format($db->cant_aju ,0,",",".")."</td>";
							
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_aju'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.gif' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_aju.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.gif' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.gif' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_aju) /></td> ";
							else
...
<form name="forma" method="post" action="con_inventario.php">
