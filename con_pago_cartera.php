...
if($eliminacion==1) {//confirmacion de insercion  
	...
	$sql ="UPDATE m_cargue SET pagado= '0',cod_car_pag= '0'  WHERE   cod_car_pag='".$eli_codigo."'";		
	$dbdatos_1->query($sql);
	$error=eliminar("m_pago_cartera",$eli_codigo,"cod_pag");
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$sql="SELECT * FROM m_pago_cartera $where  ORDER BY fec_pag DESC ";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_pago_cartera.php">
                    ...
                    <tr>
                      <td bgcolor="#E9E9E9"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/page.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_pago_cartera.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          ...
                          <td width="160" valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="che_pag">Cheque</option>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
                        <tr>
						  ...
						  <td width="138"  class="ctablasup" >FECHA </td>
						  <td width="169"  class="ctablasup">VALOR</td>
						  <td width="188"  class="ctablasup">NUMERO CHEQUE </td>
                          <td  class="ctablasup"  width="119">OPCIONES</td>
                        </tr>
						<? 
						
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_pago_cartera.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_pag' action='man_pago_cartera.php'>  ";
                          	echo "<td >$db->fec_pag</td>";
							echo "<td align='right'>".number_format($db->val_pag,0,",",".")."</td>";
							echo "<td  align='right'>$db->che_pag </td>";
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_pag'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.gif' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_pag.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.gif' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.gif' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_pag) /></td> ";
							else
								echo"<img src='imagenes/e_icoeliminar.gif' width='16' height='16'  /></td> ";
							//impresion	
							echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\" onClick=\"imprimir_inf('informe_pagos.php',$db->cod_pag,'mediano')\" /></td>";	
...
<form name="forma" method="post" action="con_pago_cartera.php">
