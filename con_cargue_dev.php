...
if($eliminacion==1) {//confirmacion de insercion  
	...
	$error=eliminar("m_dev_entrada",$eli_codigo,"cod_mdent");
	
	$sql ="DELETE FROM cartera_compras WHERE cod_doc_ccom=$eli_codigo and cod_tdoc_ccom=2";//=		
	$dbdatos_1= new  Database();
	$dbdatos_1->query($sql);
	

	$sql ="DELETE FROM d_dev_entrada WHERE cod_ment_ddent=$eli_codigo";//=		
	$dbdatos_1= new  Database();
	$dbdatos_1->query($sql);
	
					
	if ($error >=1)
	echo "<script language='javascript'> alert('Se Elimino el registro Correctamdente..') </script>" ;
	
}


if($confirmacion==1) //confirmacion de insercion 
	echo "<script language='javascript'> alert('Se Inserto el registro Correctamdente..') </script>" ;

if($confirmacion==2) //confirmacion de insercion 
	echo "<script language='javascript'> alert('Se Edito el registro Correctamdente..') </script>" ;
...
//if($det==0)
//	$where.=" where cod_mdent>0   and  ( $where_cli )  ";
...
$sql="SELECT * FROM m_dev_entrada INNER JOIN proveedor ON m_dev_entrada.cod_prove_mdent=proveedor.cod_pro $where ORDER BY cod_mdent DESC";
                    ...
                    <tr>
                      <td bgcolor="#E9E9E9"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/page.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_dev_cargue.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          ...
                          <td width="160" valign="middle"><select name="campos" class="textfieldlista" id="campos" >
							...
							<option value="fac_mdent">Factura</option>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_mdent' action='man_dev_cargue.php'>  ";
							
							
							
                          	echo "<td >$db->fec_mdent</td>";
							echo "<td >$db->fac_mdent</td>";
							echo "<td >$db->nom_pro </td>";
							
							echo "<td align='right'  >".number_format($db->total_mdent ,0,",",".")."</td>";
							
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_mdent'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.gif' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_mdent.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.gif' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.gif' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_mdent) /></td> ";
							else
								echo"<img src='imagenes/e_icoeliminar.gif' width='16' height='16'  /></td> ";
							
							//impresion	
							echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\" onClick=\"imprimir_inf('inf_dev_cargue.php',$db->cod_mdent,'mediano')\" /></td>";	
...
<form name="forma" method="post" action="con_cargue_dev.php">
