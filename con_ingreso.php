...
if($eliminacion==1) {//confirmacion de insercion  
	$error=eliminar("ingreso",$eli_codigo,"cod_ing");
	reverzar_referencias_compras ($eli_codigo);
...
$where=" where $busquedas   and cod_proy_ing=$global[0] ";
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where  cod_proy_ing=$global[0] ";


 $cantidad_paginas=paginar('SELECT cod_ing , fec_ing, alm_ing , cod_prov_ing,  nom_alm FROM ingreso
				INNER JOIN almacen ON  ingreso.alm_ing=almacen.cod_alm  '.$where.'' );
                  ...
                  <table width="624" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td bgcolor="#E9E9E9"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/page.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_ingreso.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
						...
						$estilo="ctablablanc";
						$estilo="ctablagris";
						$sql='SELECT cod_ing , fec_ing, alm_ing , cod_prov_ing,  nom_alm FROM ingreso INNER JOIN almacen ON  ingreso.alm_ing=almacen.cod_alm '.$where.'  order by cod_ing desc   '.$paginar ; 
						//echo $sql;
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_ing' action='man_ingreso.php'>  ";
                          	echo "<td >".completar($db->cod_ing,6)."</td>";
							echo "<td >$db->fec_ing </td>";
							echo "<td >$db->nom_alm </td>";
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_ing '>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.gif' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_ing.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.gif' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.gif' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_ing) /></td> ";
							else
...
<form name="forma" method="post" action="con_ingreso.php"><input type="hidden" name="editar" id="editar" value="<?=$editar?>">
